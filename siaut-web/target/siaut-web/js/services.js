angular.module("starter.services", [])
    ...
    .factory('EndPoint', function ($timeout, $window, Log, Alert, Feedback, Error) {
        var VERSION = '1.0.4';
        var BASEADDRESS = document.URL + "";
        var LOCALADRESS1 = "localhost";
        var LOCALADDRESS2 = "192.168";

        var protocolo = location.protocol;
        if (protocolo.indexOf("file") >= 0 ) {//running local on android
            protocolo = "http:";
        }
    ...
    .factory('Alert', function ($rootScope, $timeout, $uibModal, $templateCache, $controller, $http, Log, Feedback, Utils) {
        ...
        return {
            ...
            showInput: function (title, message, defaultvalue, response) {
                Log.debug("showConfirm(" + title + "," + message + "," + defaultvalue + ")");
    ...
    .factory('Analytics', function (Log, $location) {
        var VERSION = '1.0.0';
        var HOME = "http://www.geradorprototipo.caixa.gov.br/analytics";
        var enabled = false;
        var key = null;
        var session = null;
        var category = "";
    ...
    .factory('Utils', function (Log,NativeInterface) {
        var VERSION = '1.0.0';
        Log.debug("Utils");
        ...
        function isNative() {
            return NativeInterface.hasSupport();
        }
        ...
        return {
            ...
            getDevice: function() {
                var device = {
                    ...
                    isAndroid: isAndroid(),
                    isIOS: isIOS(),
                    isNative: NativeInterface.hasSupport()
                }
                return device;
            },
