package br.gov.caixa.sites.generic;
...
public class GenericTest extends Helper {
	...
	protected static final long TEMPO_SLEEP = 6500;
	protected static final long TEMPO_SPLASH = 12000;
	private static final String SENHA_PADRAO_AUTORIZACAO = "1";
	...
	protected void inputDataModalById(String idElement, String text){		
		getWebDriver().findElement(By.id(idElement)).sendKeys(text);
		JavascriptExecutor jse = (JavascriptExecutor)getWebDriver();		
		jse.executeScript("var divs = document.getElementsByClassName(\"show-calendar\");for(var i=0; i<divs.length; i++){ divs[i].style.visibility=\"hidden\"; }");
	}
	...
	protected void autorizacaoSiogp() throws InterruptedException {
		getWebDriver().findElement(By.xpath("//input[@type='password']")).clear();
		getWebDriver().findElement(By.xpath("//input[@type='password']")).sendKeys(SENHA_PADRAO_AUTORIZACAO);
